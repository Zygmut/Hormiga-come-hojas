...
public class Main extends JFrame implements KeyListener {
    ...
    public static void main(String[] args) {
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                Main frame = new Main();
                frame.setTitle("Juego de la hormiga de mierda esta que me tiene hasta la polla");
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == 32) { //space
            ...
            String orientation = board.cBoard[xpos][ypos].piece.getId().split("_")[1]; //orientacion de la hormiga
            switch (orientation) {
                case ("n"):
        ...
        } else if (e.getKeyCode() == 37 || e.getKeyCode() == 38 || e.getKeyCode() == 39 || e.getKeyCode() == 40) { //Arrow keys
            boolean found = false;
            for (int i = 0; i < Board.Dim && !found; i++) {
                for (int j = 0; j < Board.Dim && !found; j++) {
                    found = board.cBoard[i][j].piece.getId().contains("hormiga");
